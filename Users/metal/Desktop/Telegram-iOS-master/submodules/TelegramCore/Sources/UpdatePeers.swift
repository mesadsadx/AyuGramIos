...
func updatePeers(transaction: Transaction, accountPeerId: PeerId, peers: AccumulatedPeers) {
    let ayugramSettings = transaction.getSharedData(AyugramSettingsKeys.ayugramSettings)?.get(AyugramSettings.self) ?? AyugramSettings.defaultSettings
    
    var parsedPeers: [Peer] = []
    for (_, user) in peers.users {
        if var telegramUser = TelegramUser.merge(transaction.getPeer(user.peerId) as? TelegramUser, rhs: user) {
            if ayugramSettings.localPremium && telegramUser.id == accountPeerId {
                var flags = telegramUser.flags
                flags.insert(.isPremium)
                telegramUser = telegramUser.withUpdatedFlags(flags)
            }
...
public func updatePeersCustom(transaction: Transaction, peers: [Peer], update: (Peer?, Peer) -> Peer?) {
    let ayugramSettings = transaction.getSharedData(AyugramSettingsKeys.ayugramSettings)?.get(AyugramSettings.self) ?? AyugramSettings.defaultSettings
    let accountPeerId = transaction.postbox?.accountPeerId // Assuming transaction has postbox
    ...
    transaction.updatePeersInternal(peers, update: { previous, updated in
        ...
        if ayugramSettings.localPremium, let telegramUser = updated as? TelegramUser {
            // We need to know the current account peer ID.
            // In updatePeers it is passed as argument.
            // In updatePeersCustom we might need to get it from somewhere.
            // Transaction has access to postbox, but accountPeerId might not be directly available.
            // Let's assume we can get it or just skip if we can't easily find it.
            // Actually, we can check if it's the current user by other means, but accountPeerId is best.
        }
