...
private func pushPeerReadState(network: Network, postbox: Postbox, stateManager: AccountStateManager, peerId: PeerId, readState: PeerReadState) -> Signal<PeerReadState, PeerReadStateValidationError> {
    if peerId.namespace == Namespaces.Peer.SecretChat {
        ...
        return combineLatest(inputPeer(postbox: postbox, peerId: peerId), postbox.transaction { transaction -> AyugramSettings in
            return transaction.getSharedData(AyugramSettingsKeys.ayugramSettings)?.get(AyugramSettings.self) ?? AyugramSettings.defaultSettings
        } |> castError(PeerReadStateValidationError.self))
        |> mapToSignal { inputPeer, ayugramSettings -> Signal<PeerReadState, PeerReadStateValidationError> in
            switch inputPeer {
